...
def run_nrn(setup, model, duration, v_init, temperature):
    ...
    neuron_cell.create_synapse(neuron_cell.dendrites[0], "AMPA").stimulate(
        start=200, number=2, interval=7
    )
    neuron_cell.create_synapse(neuron_cell.dendrites[0], "NMDA").stimulate(
        start=200, number=2, interval=7
    )
    neuron_time = p.time
...
def run_model(model, duration=1000, v_init=-65, setup=None, temperature=305.15):
    ...
    return run_nrn(setup, model, duration, v_init, temperature), ([], [])
...
if __name__ == "__main__":
    ...
    for name, setup in models.items():
        ...
        for setup in setups:
            ...
            go.Figure(
                [
                    go.Scatter(x=nt, y=nv, name=f"NEURON: {sname}"),
                    go.Scatter(x=at, y=av, name=f"Arbor: {sname}"),
                ],
