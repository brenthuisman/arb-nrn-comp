...
with contextlib.ExitStack() as stack:
    ...
    trees = dict(
        zip(
            [f.split("ection__")[1].split(".")[0] for f in fnames],
            map(driver.parse_string, (f.read() for f in files)),
        )
    )
...
def get_state_complexity(program):
    return sum(
        ...
        for state in lookup_visitor.lookup(program, ast.AstNodeType.STATE_BLOCK)
    )


def get_param_complexity(program):
    return sum(
        len(param.statements)
        for param in lookup_visitor.lookup(program, ast.AstNodeType.PARAM_BLOCK)
    )


def get_assigned_complexity(program):
    return sum(
        len(assigned.definitions)
        for assigned in lookup_visitor.lookup(program, ast.AstNodeType.ASSIGNED_BLOCK)
    )
...
def get_method(program):
    try:
        return list(lookup_visitor.lookup(program, ast.AstNodeType.SOLVE_BLOCK))[
            0
        ].method.get_node_name()[:6]
    except IndexError:
        return ""
