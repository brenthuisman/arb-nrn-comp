...
def run_nrn(setup, model, duration, v_init, temperature):
    ...
    dur = time() - t
    return dur, neuron_time, np.array(list(neuron_cell.Vm))
...
def run_arb(setup, model, duration, v_init, temperature):
    ...
    t = time()
    sim.run(tfinal=duration, dt=0.025)
    duration = time() - t
    spikes = sim.spikes()
    data, meta = sim.samples(handle)[0]
    return duration, data[:, 0], data[:, 1]
...
if __name__ == "__main__":
    ...
    for name, setup in models.items():
        ...
        for setup in setups:
            ...
            import gc
            for i in range(20):
                adur, at, av = run_nrn(setup, model, duration=10, v_init=-65, temperature=305.15)
                gc.collect()
                dur_avg += adur
                print(sname, "\t", adur)
